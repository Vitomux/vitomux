...
function getPersonas( ){
    ...
    let valorIngresado = parseInt(personasInput.value)
        ...
        if ( isNaN(valorIngresado) ) {
                ...
                Toastify({
                text: "Revise valores ingresados",
                duration: 1000,
                ...
                Toastify({
                text: "Personas sumadas correctamente",
                duration: 1000,
                destination: "https://github.com/apvarun/toastify-js",
                newWindow: true,
                close: true,
...
async function getData( ) {
    ...
    const result = await Swal.fire({
        title: "¿Desea calcular los gastos?",
        showDenyButton: true,
        confirmButtonText: "Si, calcular",
        confirmButtonColor: "#00A18E",
        denyButtonText: `No`,
        denyButtonColor: "#D83D00"
    });

    if ( result.isConfirmed ){

        Swal.fire({
            title: 'Calculando aportes...',
        ...
        voluntarios.forEach(( voluntario )=>{
            let parteVoluntario = ( voluntario.dinero - partePorPersonas ).toFixed( 2 );
...
function save( ){
    Swal.fire({
        title: "¿Desea guardar la operación?",
        showDenyButton: true,
        confirmButtonText: "Si, guardar",
        confirmButtonColor: "#00A18E",
        denyButtonText: `No es necesario`,
        denyButtonColor: "#D83D00"
        ...
        if (result.isConfirmed) {
            ...
            `
                restart( ) 
                Swal.fire({
                    title: "Guardaste la operacion en el historial",
                    icon: "success",
                    confirmButtonColor: "#00A18E",
            });
...
function deleteHistory( ){
    Swal.fire({
        title: "¿Seguro desea borrar el historial?",
        showDenyButton: true,
        ...
        if (result.isConfirmed) {
                ...
                localStorage.clear( )
                Swal.fire({
                    title: "Borraste el historial con éxito",
                    icon: "success",
                    confirmButtonColor: "#00A18E",
            });
        } else if (result.isDenied) {
...
function volExito( ){
    Toastify({
    text: "Voluntario sumado correctamente",
    duration: 1000,
    destination: "https://github.com/apvarun/toastify-js",
    newWindow: true,
    close: true,
...
function volFail( ){
    Toastify({
    text: "Reingrese datos correctamente",
    duration: 1000,
    destination: "https://github.com/apvarun/toastify-js",
    newWindow: true,
    close: true,
...
buttons.forEach( button => {
    ...
    button.addEventListener( 'click', async function () {
            ...
            if (button.innerText === "Sumar Voluntarios") {
                ...
                Swal.fire({
                    title: "¿Seguro desea reiniciar la operación?",
                    showDenyButton: true,
                    confirmButtonText: "Si, reiniciar operación",
                    confirmButtonColor: "#00A18E",
                    denyButtonText: `No`,
                    denyButtonColor: "#D83D00"
                }).then((result) => {
                    ...
                    if (result.isConfirmed) {
                        restart()
                        Swal.fire({
                            title: "Todos los valores volvieron a cero",
                            icon: "success",
                            confirmButtonColor: "#00A18E",
                        })
                    } else if (result.isDenied) {
